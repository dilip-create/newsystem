    ...
    public function indexSettleHistoryMerchant(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = SettleRequest::query()
                ...
                ->editColumn('status', function ($data) {
                    if ($data->status == 'success' || $data->status == 'Success' || $data->status == 'SUCCESS') {
                        return '<span class="text-success text-uppercase"><b>' . trans('messages.Success') . '</b></span>';
                    }elseif ($data->status == 'approved' || $data->status == 'Approved' || $data->status == 'APPROVED') {
                            return '<span class="text-success text-uppercase"><b>' . trans('messages.Success') . '</b></span>';
                    }elseif ($data->status == 'paid' || $data->status == 'Paid' || $data->status == 'PAID') {
                            return '<span class="text-success text-uppercase"><b>' . trans('messages.Success') . '</b></span>';
                    } elseif ($data->status == 'pending' || $data->status == 'Pending' || $data->status == 'PENDING') {
                        return '<span class="text-primary text-uppercase"><b>' . trans('messages.pending') . '</b></span>';
                    } elseif ($data->status == 'processing') {
                        return '<span class="text-warning text-uppercase"><b>' . trans('messages.processing') . '</b></span>';
                    } else {
                        return '<span class="text-danger text-uppercase"><b>' . trans('messages.Failed') . '</b></span>';
                    }
                })
                ...
                ->addColumn('action', function ($data) use ($request) {
                    $action = '
                        <a class="btn btn-primary waves-effect waves-light" href="#" data-toggle="modal" data-target="#edit_user' . $data->id . '">' . trans("messages.View") . '</a>
                    ';
                    ...
                    $action .= '<tr>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>' . $data->payment_method . '</td>
                                                    <td style="width: 25%; background-color: #6c6c70 !important; color: white;">' . trans("messages.Send Notification") . '</td>
                                                    <td><a class="btn btn-danger waves-effect waves-light" href="/sendWithdrawNotification/' . base64_encode($data->id) . '" target="_blank">' . trans("messages.Click here to send Notification") . '</a></td>
                                                </tr>
                                            </table>
                                        </div>
                                    </div>
